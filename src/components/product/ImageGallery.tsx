...
const ImageGallery = ({open, setOpen}: TProp) => {
  ...
  const {data: photos, isLoading, error, refetch: refetchGallery} = useGetAllImagesQuery(parentFolder);
                ...
                <div className="absolute bottom-0 flex w-full justify-between transition-all">
                  ...
                  <Dialog open={deleteFolderOpen} onOpenChange={setDeleteFolderOpen}>
                    ...
                    <DialogContent>
                      ...
                      <div className="flex flex-col gap-4">{/* <Button onClick={() => handleDeleteFolder(folder._id)}>Edit</Button> */}</div>
                    </DialogContent>
                  </Dialog>
                </div>
              )}
  ...
  const uploadPhotoElement = (
    <div className="flex h-36 justify-center items-center flex-col gap-3">
      <div
        onClick={handleUploadClick}
        className="border-2p min-w-44 px-3 cursor-pointer flex-1 w-1/3 border-dashed h-full rounded-md border-lavender-mist bg-background flex flex-col justify-center items-center"
      >
        {isUploadLoading ? (
          <div className="flex flex-col gap-2 justify-center items-center">
            <AiOutlineLoading3Quarters className="animate-spin text-primary lg:text-6xl text-3xl" />
            <p className="text-base">uploading, please wait</p>
          </div>
        ) : (
          <>
            <h3 className="text-base text-center">upload photos to this folder</h3>
            <AiOutlineCloudUpload className="text-lavender-mist lg:text-6xl text-3xl" />
    ...
    <>
      <Dialog open={open} onOpenChange={() => setOpen(!open)}>
        <DialogContent className="lg:max-w-[70vw] md:max-w-[80vw] sm:max-w-[85vw] max-w-[95vw]">
          ...
          {selected.length > 0 && (
            <div className="sticky -top-6 z-50 bg-background-foreground px-3 flex justify-between items-center py-2">
              ...
              <div className="flex gap-3">
                ...
                <Button variant={"edit"} className="gap-2" onClick={() => setSelected([])}>
              ...
              <BreadcrumbList>
                {folderCrumb.map((item, index) => (
                  <BreadcrumbItem key={item.id}>
                    <BreadcrumbPage>
                      <div
                        className={`flex items-center gap-1 cursor-pointer ${item.id === parentFolder ? "text-primary" : ""}`}
          ...
          {!isFolderLoading && !folderFetchError && folders?.data?.length !== 0 ? (
            renderFolders(folders.data)
          ) : (
            <div
              onClick={() => setAddFolderModal(true)}
              className="bg-lavender-mist size-32 rounded-md flex flex-col gap-2 justify-center items-center"
            >
              <LuFolderPlus className="text-gray" size={25} />
              <p className="text-sm">Add Folder</p>
            </div>
          ...
          <div>
            ...
            {!isLoading && !error ? (
              <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 [&>div:not(:first-child)]:mt-4 rounded-md min-h-60">
                {photos?.data?.length > 0 && uploadPhotoElement}
                {photos?.data?.map((imgData: TImage) => {
                  return (
                    <div key={imgData._id} className="relative group h-fit image-wrapper">
                      <Checkbox
                        checked={isSelected(imgData._id)}
                        onClick={() => handleImageSelect(imgData)}
                        className={`${
                          isSelected(imgData._id) ? "visible" : "invisible"
                        } transition-all absolute top-4 left-4 checkbox group-hover:visible size-6 bg-lavender-mist`}
                      />
                      <Image
                        className="h-auto cursor-pointer w-full rounded-md border border-border-color"
                        src={imgData.image}
                      ...
                      <p className="absolute invisible group-hover:visible z-30 text-xs text-pure-white bg-purple-400/30 bottom-0 p-4 break-all w-full">
                        {imgData.name}
                      </p>
                    </div>
                  );
