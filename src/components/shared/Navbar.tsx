  ...
  return (
    <nav className="">
      <div className="bg-white px-4 py-4 shadow-md rounded-md mt-3 flex justify-between">
        {!isDesktopOrLaptop && (
          <Button className="px-0 text-2xl" onClick={() => dispatch(setMenuOpen(true))} variant={"icon"}>
        ...
        {isAuthenticated && (
          <div className="flex justify-end ms-auto">
                ...
                <Button variant="icon">
                  <Avatar>
                    ...
                    <AvatarFallback className="capitalize">
                      {user?.name.firstName.slice(0, 1)} {user?.name.lastName.slice(0, 1)}
                    </AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="w-72 pb-4 -right-10">
                <DropdownMenuLabel>
                  <div className="bg-primary rounded-tl-md rounded-tr-md text-pure-white px-3 py-4 flex justify-between items-center">
                    ...
                    <Button variant={"delete_solid"} className="bg-background text-red" onClick={handleLogout}>
                      Logout
                    </Button>
                  </div>
                </DropdownMenuLabel>
            ...
            {loaded && (
              <div className="flex justify-center items-center border border-border-color w-fit mx-auto rounded-full shadow-lg bg-lavender-mist font-semibold">
                <button
                  onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
                  className={`flex text-black items-center gap-2 px-3 py-2 h-full rounded-full ${
